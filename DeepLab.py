...
class DeepLabModel(object):
    ...
    def __init__(self, tarball_path):
        ...
        print('Extract frozen graph from tar archive.')
...
def vis_segmentation(image, seg_map, combine_img):
    """Visualizes input image, segmentation map and blurred background view."""
...
if __name__ == '__main__':
    THRESHOLD_VALUE = 1
    deeplab = DeepLabModel('models/deeplabv3_mnv2_pascal_trainval_2018_01_29.tar.gz')
    test_raw_img = Image.open('imgs/girl.png')

    resized_raw, seg_map = deeplab.run(test_raw_img)
    ...
    result = np.bitwise_or(masked_front_img, masked_bkg_img)
    vis_segmentation(resized_raw, seg_map, result)
